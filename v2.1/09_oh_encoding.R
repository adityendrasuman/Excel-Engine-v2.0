tryCatch(
  {
    ...
    purpose <- "Splitting all the 'MULTISELECT' question columns into constituent columns with Yes/No values"
    ...
    if (is.null(nrow(col_list))) {
      ...
      if (nrow(summary3_temp) > 0){
        ...
        for (q in questions) {
          ...
          list_of_columns <- dt_01_C %>%
            select(matches(paste0("^.*", q, ".*$"))) %>%
            colnames() %>% 
            intersect(columns) %>% 
            unlist()
          
          table_with_relevant_cols <- dt_01_C %>%
